...
`writablePaths`·`protectedPaths` 같은 경로 규칙은 파일을 만드는 `write_file`·`edit_file`뿐 아니라 파일을 지우는 `delete_file` 호출에도 똑같이 적용됩니다. 지우는 것도 그 서비스의 코드를 바꾸는 일이므로 범위와 보호 경로를 그대로 봅니다.
...
| 작업별 쓰기 범위 (`writablePaths`) | 도구 호출 직전. 작업 분해가 작업마다 `studio.yaml` 정책 위에 더함 | 범위 밖 `write_file`·`edit_file`·`delete_file`은 막히고, 명령으로 만든 범위 밖 파일은 통합 직전에 다시 걸러집니다 |
...
- Pi 경로의 변경은 호스트 작업 폴더에서 일어나며 b-studio 샌드박스·검증 게이트·체크포인트를 거치지 않습니다. 이 확장은 개인 Pi 사용에서 팀 규칙을 먼저 적용하는 안내·조기 차단 계층입니다. 다만 Pi나 편집기로 바꾼 현재 작업 트리의 변경은 `studio verify <프로젝트 경로>`로 에이전트와 같은 검증 게이트(재시작·준비 판정·계약 비교·화면 확인·테스트·리뷰)에 넣어 확인할 수 있습니다. 이 명령은 판정만 하고 체크포인트나 커밋을 만들지 않으므로, 체크포인트와 배포가 필요하면 스튜디오 세션에서 실행해야 합니다. 계약 비교 기준은 이 명령을 돌리는 시점의 현재 코드라서, 편집기에서 이미 깬 계약은 이 명령으로 잡지 못합니다.
