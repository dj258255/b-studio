...
- **Gradle**: 템플릿의 `gradle.properties`로 빌드 캐시(공유 `GRADLE_USER_HOME`에 저장)와 설정 캐시(프로젝트 `.gradle`에 저장, 스냅샷으로 재사용)를 켰습니다. 설정 캐시와 맞지 않는 플러그인이 있어도 빌드를 막지 않도록 문제는 경고로만 남깁니다.
