...
- `browser_check`는 HTTP 응답의 상태 코드와 문구만 봅니다. 헤드리스 브라우저 렌더링, 클라이언트 스크립트 오류, 레이아웃은 확인하지 않습니다.
...
- 배포 조건 거부(409)는 `releaseBlockers` 단위 테스트와 트레일러 Git 왕복 테스트(본문 8,000자 초과, 요약 속 위조 줄 포함)로 확인했고, 웹 배포 API를 통한 전 과정 검증은 아직 없습니다.
- 원격 동기화로 가져온 커밋의 `Workflow-Passed` 트레일러는 원격 작성자가 적은 그대로 읽습니다.
