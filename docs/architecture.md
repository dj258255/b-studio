...
| 작업 분해 | 한 요청을 여러 작업으로 나눠 동시에 처리 | 이어진 작업은 한 세션(레인)에서 차례로, 레인끼리는 다른 세션에서 동시에. 작업마다 쓰기 범위를 실행기에서 걸고, 통합 결과를 새 세션에서 게이트로 다시 검증 |
...
```text
요청 ─ 계획(모델 JSON) ─ planLanes 검증 ─┬─ lane-1 세션: a1 → a2  (쓰기 범위 web/a)
