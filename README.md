...
- **작업 분해**: 한 요청을 모델이 작업·쓰기 범위·의존 관계로 나누면 스튜디오가 검증합니다. 이어진 작업은 한 세션에서 차례로, 독립 레인은 다른 세션에서 동시에 돌리고, 작업마다 쓰기 범위를 실행기에서 걸어 레인끼리 변경이 겹치지 않게 합니다. 결과는 새 세션에서 같은 게이트로 다시 적용해 합친 뒤 검증합니다.
