...
- **작업 그래프**: 서로 기다릴 필요가 없는 검증 작업(테스트, 화면 확인)을 동시 실행 수·재시도·실패 전파 규칙으로 병렬 처리합니다. 검증 게이트가 이 그래프로 `studio.yaml`에 선언한 검사를 돌립니다.
...
| 병렬 Task Graph · 의존성·재시도·실패 전파 (게이트의 테스트·화면 확인에 사용) | 완료 |
| 워크플로 강제: 테스트·화면 확인·리뷰 단계, 필수 단계 대조, 배포 조건 | 완료 |
| Pi 정책 확장 (Pi 내장 도구 차단·안내) | 완료. Pi 경로는 체크포인트를 만들지 않음 |
