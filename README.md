...
| 8. 데이터 보호 · 정책 프록시 · 리소스 한도 · 격리 강화 | 📋 계획 |
...
| 단위 테스트 / 타입 체크 | 117개 통과 / 패키지 5개 통과 |
...
- [ ] **데이터 보호**: 운영 DB 접근 차단(네트워크 격리), 시크릿 주입과 가림
- [ ] **정책 프록시**: 사내 API 등록, 마스킹, 감사 로그, 서비스 단위 권한
- [ ] **리소스 한도와 사용량 표시**: 서비스별 메모리 한도, CPU·메모리 사용량과 재시작·종료 코드(메모리 부족 종료 표시), 지금 실행 중인 단계(Gradle 태스크 등), 에이전트용 사용량 도구
