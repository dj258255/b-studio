...
export B_STUDIO_AUTH_TOKENS="alice:$(openssl rand -hex 24),bob:$(openssl rand -hex 24)"   # token: 사람마다 접근 토큰 (24자 이상)
...
만든 앱은 운영 이미지로 빌드해 같은 Docker 호스트에 배포할 수 있습니다. managed 서비스 폴더에 운영용 `Dockerfile`이 있어야 합니다(템플릿과 예제에 들어 있음). 새 릴리스가 준비되면 고정 주소를 무중단으로 바꾸고, DB 같은 부가 서비스는 데이터를 유지한 채 그대로 둡니다.
...
- **코드 보기의 범위**: 읽기 전용이고 목록은 파일 500개까지이며, 찾기도 그 안에서 경로로만 합니다. 파일 변경 감시는 macOS에서만 실측했고, Linux에서는 폴더가 아주 많으면 inotify 감시 한도에 걸릴 수 있습니다. 로컬 로그인 계정 모드에서는 실제 모델로 확인하지 않았습니다.
...
- **답변 마크다운의 범위**: GFM(표·취소선·작업 목록)까지 그리고, 수식·각주는 지원하지 않습니다. 답변 속 이미지는 불러오지 않습니다.
- **세션 토큰 한도의 범위**: 로컬 로그인 계정 모드는 턴을 끝낼 때만 사용량을 알려 주므로 한도를 넘은 뒤에 멈추고, 넘은 만큼은 이미 쓴 토큰입니다. 캐시 읽기도 다른 토큰과 똑같이 세는 토큰 수 한도이며 금액 한도가 아닙니다. 사람·팀 단위 한도는 없고, API 키 모드에서는 실제 모델로 확인하지 않았습니다.
- **스튜디오 인증의 범위**: 토큰 모드의 세션은 서명 쿠키라 로그아웃해도 복사해 둔 쿠키는 만료 전까지 유효합니다(모두 무효로 하려면 서명 키를 바꿈). 토큰은 환경 변수에 평문으로 두고, 계정 잠금 없이 틀린 토큰에 0.5초 지연만 둡니다. 권한은 세션 단위(만든 사람, 관리자)뿐이고, 미리보기 게이트웨이와 서비스 포트는 이 인증을 거치지 않습니다. proxy 모드는 헤더를 직접 넣어 확인했고 실제 SSO 프록시(oauth2-proxy 등) 뒤에서는 띄워 보지 않았습니다.
- **운영 배포의 범위**: 스튜디오와 같은 Docker 호스트에만 배포하고, 운영 주소는 127.0.0.1에만 공개합니다. 다른 PC에서 쓰려면 앞단 리버스 프록시와 TLS가 필요합니다. 운영 설정(DB 비밀번호 등)은 개발용 compose의 값을 그대로 쓰고, 되돌리기는 DB 스키마를 되돌리지 않습니다. 지금은 CLI로만 배포하며, 스튜디오 화면의 배포는 아직 없습니다. Spring Boot·Next.js는 orders 예제로 배포까지, FastAPI 템플릿은 운영 이미지 실행까지만 확인했습니다.
...
- **원격 미리보기의 범위**: 게이트웨이는 루프백 바인드와 `preview.localhost`로 같은 PC에서만 실측했고, 다른 PC·실제 와일드카드 DNS·TLS 리버스 프록시·Kubernetes 제공자로는 확인하지 않았습니다. 게이트웨이는 스튜디오 인증을 거치지 않아, 토큰이 든 주소를 아는 사람은 누구나 미리보기를 볼 수 있습니다.
