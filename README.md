...
| 코드 | 샌드박스에서 도는 프로젝트 파일을 봅니다. 마지막 체크포인트 이후 추가·수정·삭제한 파일을 먼저 보여 주고 파일마다 변경 내용을 볼 수 있습니다. 에이전트가 파일을 쓰거나 고치면 요청이 끝나기 전에 바로 반영하고, 그 파일로 따라갑니다. 생성물과 `.env`는 에이전트 작업 공간과 같은 규칙으로 빼고, 시크릿 값은 가려서 보여 줍니다 |
