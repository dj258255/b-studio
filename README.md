...
- **같은 프로젝트의 동시 세션**: 같은 프로젝트로 두 세션을 동시에 띄우면 Gradle 캐시 볼륨을 공유하기 때문에 뒤에 뜨는 세션의 api 빌드가 잠금을 얻지 못해 실패합니다(메모리 문제가 아니라 캐시 잠금입니다. [트러블슈팅 36](docs/troubleshooting.md#36-같은-프로젝트로-두-세션을-동시에-띄우면-api-빌드가-실패함)). 같은 프로젝트는 한 번에 한 세션만 띄우세요. 다른 프로젝트끼리는 캐시가 달라 영향이 없습니다.
