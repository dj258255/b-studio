...
| 대화 | 요청, 에이전트 답변, 도구 사용 기록, **검증 게이트** 결과가 순서대로 쌓입니다. 처리 중인 요청은 샌드박스를 그대로 둔 채 취소해 변경을 되돌릴 수 있고, 요청마다와 세션 전체의 모델 토큰 사용량을 보여 줍니다. 에이전트 답변의 표·목록·코드 블록은 서식대로 그리고, 답변 속 HTML은 글자로 둡니다 |
