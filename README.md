...
# 객체에서 methods와 paths를 적지 않으면 GET·HEAD와 모든 경로가 기본값이다.
# network:
#   egress:
#     - api.slack.com
...
덤프를 문자열이 아니라 파일로 흘리게 바꾼 뒤, 예전 상한(256MB)을 넘기는 크기로 다시 쟀습니다. 세션을 띄운 다음 `db`에 1KB 행 30만 개(테이블 341MB)를 넣고 데모 요청으로 체크포인트를 만들었습니다.

| 확인 항목 | 결과 |
|---|---|
| 세션 시작 덤프 | 1,951바이트 |
| 큰 데이터 넣기 | 300,000행 3.0초, 테이블 341MB |
| **256MB를 넘는 덤프 저장** | 덤프 파일 302,292,246바이트(288.3MiB). 데모 요청이 끝나기까지 34.1초. 예전 경로는 문자열 버퍼 상한 256MiB에 걸려 저장이 실패하던 크기 |
| 288MiB 덤프로 되돌리기 | 절반(150,000행)을 지운 뒤 그 체크포인트로 복원해 300,000행 회복. DB 행이 돌아오기까지 6.8초(서비스 재시작 완료까지는 재지 않음) |
| 임시 파일 | 끝난 뒤 `.tmp` 0개, 덤프 파일 3개(세션 시작 + 체크포인트 2개) |
| 기동·자원 | 세션 준비 28.9초. dbtower 컨테이너 11개가 도는 6GiB VM에서 실행해 전후 모두 11개 살아 있음(가용 메모리 1,769→1,807MiB, 디스크 39%→40%) |
...
경로 규칙이 퍼센트 인코딩으로 우회되는지도 따로 쟀습니다. `allow: [web, studio] GET /api/users/*`만 적고, 가짜 사내 API가 **받은 경로를 모두 기록**하게 했습니다.

| 확인 항목 | 결과 |
|---|---|
| 규칙에 맞는 경로 | `GET /api/users/1` → 200, `allow` |
| 인코딩된 구분자 | `GET /api/users/..%2Fadmin` → 403, 이유 "경로에 인코딩된 구분자가 있어 규칙을 검사할 수 없습니다" |
| 이중 인코딩 | `GET /api/users/..%252Fadmin` → 403, 같은 이유 |
| 인코딩된 역슬래시 | `GET /api/users/7%5Cadmin` → 403 |
| 상위 유출 | 가짜 사내 API가 받은 경로는 `/api/users/1` 하나뿐. 막힌 요청 3건은 닿지 않음 |
| 기동 | 30.7초 |
...
- **DB 브랜치의 한계**: Postgres만 지원합니다. 덤프를 파일로 흘려 크기 상한은 없앴지만(288MiB로 실측), 체크포인트마다 덤프 하나가 디스크에 쌓이고 저장·비교·복원마다 그 크기를 읽고 씁니다. 되돌림 결과 문구는 이벤트 스트림으로 확인했고 브라우저 화면으로는 확인하지 않았습니다. 큰 덤프 되돌리기는 DB 행이 복원되는 시점까지 쟀고, 서비스 재시작 완료까지는 재지 않았습니다.
...
- **정책 프록시의 범위**: 필드 이름(`mask`)과 값 형태(`maskPatterns`)로 가립니다. 값 형태는 전화·메일·주민번호·카드 4가지만 알아서, 사번·계좌번호처럼 조직마다 형태가 다른 값은 필드 이름으로 적어야 합니다. `studio.yaml`은 에이전트가 고칠 수 있는 파일이라 임의 정규식은 받지 않습니다. 가릴 규칙이 있는 API의 JSON이 아닌 응답은 넘기지 않고, 본문은 5MB까지, HTTP(S) API만 다룹니다. 값 형태 가림은 studio 호출자 경로로만 다시 쟀고(가림은 서비스 호출자와 같은 함수를 지납니다), 실제 사내망 API가 아니라 Docker 호스트의 가짜 API로 확인했습니다. 경로에 인코딩된 구분자가 든 요청은 허용 규칙과 무관하게 거부하므로, 경로에 인코딩된 `/`를 쓰는 API는 이 프록시로 부를 수 없습니다. 이 거부도 studio 호출자 경로로 실측했고 서비스 호출자 경로는 같은 함수를 지납니다.
...
- **네트워크 격리의 범위**: 외부로는 허용한 호스트의 HTTP(S)만 나갈 수 있습니다. `network.egress`의 문자열 규칙은 호스트 전체를 열고, 객체 규칙은 평문 HTTP의 메서드·경로까지 제한합니다. HTTPS는 `CONNECT` 터널이라 경로·메서드를 볼 수 없어 객체 규칙만으로는 열지 않고, 호스트 전체 허용이 필요합니다. 경로에 퍼센트 인코딩된 구분자(`%2f`, `%5c`, 이중 인코딩 `%25`)가 있으면 견줄 수 없다고 보고 거부하므로, 그런 경로가 정말 필요한 API도 호스트 전체를 열어야 합니다(egress 쪽 거부는 단위 테스트로만 확인했습니다). 프록시 변수(`HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`와 소문자)는 넣지만 이를 모두 무시하는 도구는 이름 풀이부터 실패하며 감사 로그에도 남지 않습니다. 사용자 compose의 `JAVA_TOOL_OPTIONS`와 프록시 변수는 override 값으로 덮어씁니다.
